...
このデータフレームはご覧のとうりwide型になっています。

これをtidy化、すなわち各列が個々の変数と各行が個々の観測、に加工するには`tidyr`の`gather()`関数を使います。もっと詳しくはHadleyの[論文](https://www.jstatsoft.org/article/view/v059i10/v59i10.pdf)を読んでください。

これをlong型(tidy型)に変えるの
...
可視化しますと日本のGPI順位が年々下がっている事が明らかになります（＊２０１７年はアイルランドと同位でしたが上記のデータクリーニングで日本が11位になりました＊）。これはもしも　　　もしかして
...
英語も読める方はぜひ他のブログ記事を読んでください。
次に翻訳するブログ記事は昨年投稿した[Thrice Part 1]()だと考えています。
