...
また`glimpse()`でデータフレームを見てみると。。。少数の国々が同じGPIランクになっている事が見えます。その国の"rank"カラムには位置の値の前に"＝"が入っていますので、この記号を`stringr`の`str_replace()`関数で消しましょう。その次に"rank"をnumericに変えて、データフレームを`arrange()`で"year", "rank"の順に変えましょう(2008: Rank #1 - #163, 2009: Rank #1 - #163, etc.)。
...
- `coord_cartesian()`で１位から１０位だけの描画領域を見せます。
- `scale_y_reverse()`
- `theme_peace`は先ほど作ったthemeです。
- `scale_color_manual()`でラインの色を指定します。
- `scales::pretty_breaks()`の変数はx-axis/y-axisのティック数を指定します。
...
まだ改善できる所がありますね。特に文字列が重なり合っている事と各ラインに色を付けてもっと分かりやすくできると思います。最初の方は`gg_repel`のパッケージで文字列が重なり合わない用にします。ついでに各国に色を指定しましょう。色は"red", "blue"等名前だけではなく、RではHEX値も使えます、どの色を選ぶかを迷っていたら[これ](http://colorbrewer2.org/)か[このadd-in](https://github.com/daattali/colourpicker)を見てください。
...
勿論、ランキングの変化はすべて国際的な理由で変わっている訳では無い、「近隣国との関係」はGPIスコアを計算する23個の指標の一つでしかありません。他には殺人事件数、GDPに対する軍事費の比率、政治不安定さ等の項目からGPIが計算されています。詳細は最新の(GPIレポート)[http://visionofhumanity.org/app/uploads/2017/06/GPI17-Report.pdf]のアペンディックスBを呼んで下さい。
...
色を付けた線とラベルで東・東南アジアのランクが徐々に下降している傾向が明らかに見えます。例外はランクを一上一下した台湾。
...
このブログ記事ではデータ収集から可視化までのワークフロー全てを記載しました。あんまり深く探りませんでしたが分析プロジェクトの第一歩、Exploratory Data Analysis (探索データ解析)、としてはいい例だと思っています。Tidyverseの関数を使い、参考になれば幸いです。
